...
def main():
    ...
    ap.add_argument("--minutes", type=float, default=45,
                    help="この分数で打ち切って結果を出す")
    ap.add_argument("--mz-frac", type=float, default=0.8,
                    help="前付けレースの割合。展示と本番が食い違うのはここだけ")
    args = ap.parse_args()
    ...
    n_mz = int(args.n * args.mz_frac)
    sample = random.sample(mz, min(n_mz, len(mz))) + \
             random.sample(no, min(args.n - n_mz, len(no)))
    random.shuffle(sample)
    print(f"raw内の前付けレース {len(mz):,}/{len(mz)+len(no):,} "
          f"({len(mz)/(len(mz)+len(no))*100:.1f}%)")
    print(f"照合 {len(sample)}レース(前付け{n_mz}件) × 2ページ  "
          f"上限{args.minutes}分\n")
    ...
    def summary():
        ...
        for k in ("そのまま vs 展示", "入れ替え vs 展示",
            ...
            print(f"    {k}   {hit[k]:>3}/{done}", flush=True)

    t0 = time.time()
    pool = ThreadPoolExecutor(max_workers=2)
    for i, (d, jcd, rno, raw) in enumerate(sample):
        if (time.time() - t0) / 60 > args.minutes:
            print(f"\n時間切れ({args.minutes}分)。{i}件で打ち切ります。")
            break
        try:
            f1 = pool.submit(get, sess, "beforeinfo", d, jcd, rno)
            f2 = pool.submit(get, sess, "raceresult", d, jcd, rno)
            tenji, e1 = f1.result()
            honban, e2 = f2.result()
        ...
        if (i + 1) % 20 == 0:
            el = time.time() - t0
            print(f"  {i+1}/{len(sample)}  {el:.0f}秒  "
                  f"残り約{el/(i+1)*(len(sample)-i-1)/60:.0f}分", flush=True)
            summary()
