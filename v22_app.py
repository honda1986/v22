...
# ---------- 締切通知（ntfy.sh のスケジュール配信を利用） ----------
# 仕組み: スキャン時に ntfy.sh へ「配信時刻つきメッセージ」をPOST → サーバー側が保持し、
# 指定時刻にスマホの ntfy アプリへプッシュ通知。アプリ側の常駐・タップ操作は不要で、
# 画面OFF・省電力中でも届く。ブラウザからOSアラームを直接セットする方式（intent:）は
# Chrome がアラーム権限を持たず端末により無反応になるため廃止した。

NTFY_TOPIC_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ntfy_topic.txt")


def sanitize_ntfy_topic(t: str) -> str:
    """ntfyのトピック名に使える文字（英数・-・_）だけ残す。"""
    return re.sub(r"[^A-Za-z0-9_-]", "", t or "")


def load_or_create_ntfy_topic() -> str:
    """保存済みトピック名を読む。無ければ推測されにくい名前を自動生成して保存。"""
    try:
        t = sanitize_ntfy_topic(open(NTFY_TOPIC_FILE, encoding="utf-8").read().strip())
        if t:
            return t
    except OSError:
        pass
    t = "kyotei-" + secrets.token_hex(4)          # 例: kyotei-3f9a12cd
    try:
        open(NTFY_TOPIC_FILE, "w", encoding="utf-8").write(t)
    except OSError:
        pass
    return t


def save_ntfy_topic(t: str) -> None:
    try:
        open(NTFY_TOPIC_FILE, "w", encoding="utf-8").write(sanitize_ntfy_topic(t))
    except OSError:
        pass


def schedule_ntfy_notifications(topic: str, date_str: str, records: List[dict],
                                minutes_before: int, sent_keys: set):
    """買い目ありレース（close付き）に対し、締切 minutes_before 分前の配信予約を ntfy.sh へ登録。
    返り値: (booked, dup, passed, failed)
      booked: [(通知時刻 'HH:MM', record), ...] 予約成功
      dup   : このセッションで予約済みだったためスキップした件数
      passed: 通知時刻をすでに過ぎていた件数 / failed: ["場R: 理由", ...]"""
    booked, dup, passed, failed = [], 0, 0, []
    now = datetime.now(JST)
    for r in sorted([x for x in records if x.get("close")], key=lambda x: x["close"]):
        try:
            dt_close = datetime.strptime(f"{date_str}{r['close']}", "%Y%m%d%H:%M").replace(tzinfo=JST)
        except ValueError:
            continue
        dt_alarm = dt_close - timedelta(minutes=minutes_before)
        if dt_alarm <= now + timedelta(seconds=15):   # ntfyは~10秒未満先を受け付けない
            passed += 1
            continue
        key = (date_str, r["jcd"], r["rno"], minutes_before)
        if key in sent_keys:
            dup += 1
            continue
        payload = {
            "topic": topic,
            "title": f"締切{minutes_before}分前",
            "message": f"{r['place']}{r['rno']}R 締切 {r['close']}（買い目{r['n_buys']}点）",
            "priority": 4,
            "tags": ["rotating_light"],
            "delay": str(int(dt_alarm.timestamp())),
        }
        try:
            resp = req_sess.post("https://ntfy.sh", json=payload, timeout=10)
            if resp.status_code == 200:
                sent_keys.add(key)
                booked.append((dt_alarm.strftime("%H:%M"), r))
            else:
                failed.append(f"{r['place']}{r['rno']}R: HTTP {resp.status_code}")
        except requests.RequestException as e:
            failed.append(f"{r['place']}{r['rno']}R: {type(e).__name__}")
    return booked, dup, passed, failed
...
with tab3:
    ...
    alarm_min = st.slider("🔔 締切通知（何分前に届くか）", 3, 30, 10, 1, key="t3_alarm_min",
                          help="当日スキャンで買い目が出たレースについて、締切予定時刻の指定分前に"
                               "スマホへプッシュ通知を予約します（ntfy.sh のスケジュール配信を利用）。")
    _saved_topic = load_or_create_ntfy_topic()
    c_nt1, c_nt2 = st.columns([1, 2])
    with c_nt1:
        notify_on = st.checkbox("🔔 通知を自動予約", value=True, key="t3_ntfy_on",
                                help="ONにすると、当日スキャンで買い目が出たレースの通知予約を自動で行います。")
    with c_nt2:
        _topic_in = st.text_input("通知トピック名（ntfyアプリで購読）", value=_saved_topic,
                                  key="t3_ntfy_topic",
                                  help="英数・ハイフン・アンダースコアのみ。推測されにくい名前にしてください"
                                       "（このトピック名を知っている人は通知を購読できます）。")
    ntfy_topic = sanitize_ntfy_topic(_topic_in)
    if ntfy_topic and ntfy_topic != _saved_topic:
        save_ntfy_topic(ntfy_topic)
    st.caption("🔔 **初回設定（2分）**: Playストアで「**ntfy**」アプリをインストール → 右下＋で "
               f"トピック「**{ntfy_topic or '(未設定)'}**」を購読 → 以後はスキャンするだけで自動予約。"
               "通知が遅れる場合は ntfy アプリの設定で「即時配信（Instant delivery）」をONに。")
    ...
    if st.button("🚀 全レース解析", type="primary", use_container_width=True, key="t3_run"):
        if not sel_jcds:
            ...
            if not hits:
                ...
                # --- 🔔 締切通知（ntfyスケジュール配信の自動予約） ---
                st.subheader(f"🔔 締切通知（締切の{alarm_min}分前にプッシュ）")
                _now = datetime.now(JST)
                no_time = [r for r in bet_records if not r.get("close")]
                if dstr3 != _now.strftime("%Y%m%d"):
                    st.caption("通知予約は**当日スキャンのときのみ**行います（過去・未来日は対象外）。")
                elif not notify_on:
                    st.caption("通知の自動予約はOFFです（スキャン設定の「🔔 通知を自動予約」でON にできます）。")
                elif not ntfy_topic:
                    st.warning("通知トピック名が未設定です。スキャン設定でトピック名を入力してください。")
                else:
                    sent_keys = st.session_state.setdefault("ntfy_sent", set())
                    booked, dup, passed, failed = schedule_ntfy_notifications(
                        ntfy_topic, dstr3, bet_records, alarm_min, sent_keys)
                    if booked:
                        st.markdown("\n".join(
                            f"- 🔔 **{at}** に通知 ─ {r['place']}{r['rno']}R"
                            f"（締切 {r['close']}・{r['n_buys']}点）"
                            for at, r in booked))
                        st.success(f"{len(booked)}件の通知を予約しました → ntfyアプリの"
                                   f"トピック「{ntfy_topic}」に届きます。"
                                   + (f" 予約済みスキップ{dup}件。" if dup else "")
                                   + (f" 締切間近のため除外{passed}件。" if passed else ""))
                    else:
                        st.info("新規に予約した通知はありません"
                                + (f"（予約済み{dup}件）" if dup else "")
                                + (f"（締切{alarm_min}分前を過ぎたレース{passed}件）" if passed else "")
                                + ("" if (dup or passed) else "（締切時刻を取得できたレースがありません）"))
                    if failed:
                        st.warning("予約に失敗したレース: " + " / ".join(failed[:8])
                                   + (" …" if len(failed) > 8 else "")
                                   + " — 通信環境を確認して再スキャンすると再試行されます。")
                    st.caption("同じレースの二重予約はこのセッション内では自動でスキップされます"
                               "（アプリを再起動して再スキャンすると再予約されます）。"
                               "テスト送信はChromeで https://ntfy.sh/" + ntfy_topic +
                               " を開いて画面から送るか、購読直後の自動メッセージで確認できます。")
                if no_time:
