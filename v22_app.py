...
# ============================================================
# 節間成績（今節成績）: kyotei.sakura.ne.jp の出走表から取得し、
# モデルの3連単確率を後段で補正する（学習済み重みではないヒューリスティック）。
# ============================================================
JCD_ROMAJI = {
...
def parse_setsu_racelist(html: str) -> Dict[int, Dict[int, str]]:
    """sakura の出走表HTMLから今節成績を取り出す。
    返り値: {rno: {lane: 着順連結文字列}}  例 {1: {1:'6', 3:'23', ...}}。
    構造: 各レース表の「今節成績」行に <div class="tdSetsu">..</div> が6艇ぶん。
    表は <a name="1R">..<a name="12R"> の順に並び、tdSetsu を6個ずつ = R1..R12。"""
    soup = BeautifulSoup(html, "html.parser")
    rnos = [int(m.group(1)) for m in re.finditer(r'name="(\d+)R"', html)]
    vals = [d.get_text(strip=True) for d in soup.find_all("div", class_="tdSetsu")]
    out: Dict[int, Dict[int, str]] = {}
    for idx in range(len(vals) // 6):
        grp = vals[idx * 6: idx * 6 + 6]
        rno = rnos[idx] if idx < len(rnos) else idx + 1
        out[rno] = {lane: grp[lane - 1] for lane in range(1, 7)}
    return out


def fetch_setsu_results(date: str, jcd: int) -> Dict[int, Dict[int, str]]:
    """指定日・場の今節成績を sakura から取得（best-effort）。取れなければ空辞書。
    1ページに全12R分が入っているため、場ごとに1回の取得で済む。"""
    ...
    try:
        ...
        if r.status_code != 200 or len(r.text) < 3000:
            return {}
        return parse_setsu_racelist(r.text)
    except Exception:
        return {}


def setsu_avg_finish(s: str) -> Optional[float]:
    """今節成績の着順連結文字列から平均着順を返す（小さいほど好調）。
    各文字=1レースの着順(1-6)。F/L は最悪(6)扱い。実績無しは None。"""
    vals = []
    for ch in (s or ""):
        if ch.isdigit():
            d = int(ch)
...
def apply_setsu_adjustment(combo_probs: Dict[str, float],
                           setsu_by_lane: Dict[int, str],
                           k: float) -> Dict[str, float]:
    """今節成績で3連単確率を補正して再正規化する（k=0で無補正）。
    好調艇(平均着順が小さい)を1着=全係数・2/3着=半係数で増幅するヒューリスティック。
    実績が2艇未満しか無い(初日など)場合は補正しない。"""
    if k <= 0 or not combo_probs:
        return combo_probs
    avg = {lane: setsu_avg_finish(setsu_by_lane.get(lane, "")) for lane in range(1, 7)}
    known = [v for v in avg.values() if v is not None]
    if len(known) < 2:
        return combo_probs
    field_mean = sum(known) / len(known)
    s = {lane: (avg[lane] if avg[lane] is not None else field_mean) for lane in range(1, 7)}
    mean_s = sum(s.values()) / 6.0
    L = {lane: k * (mean_s - s[lane]) for lane in range(1, 7)}   # 着順が小さいほど正
    adj = {}
    for combo, p in combo_probs.items():
        try:
            a, b, c = (int(x) for x in combo.split("-"))
            ...
            adj[combo] = p
            continue
        adj[combo] = p * float(np.exp(L[a] + 0.5 * L[b] + 0.5 * L[c]))
    tot = sum(adj.values())
    if tot > 0:
...
with tab3:
    ...
    st.caption("⚠️ 選択場×12レースを順に取得して解析します。全場だと数分かかることがあります。"
               "オッズは取得しないため EV 判定は行いません（勝率＝モデルの3連単確率）。"
               "結果が出ているレースは三連単の結果・払戻を表示し、確定レースのみで当日回収率を集計します。")
    cs1, cs2 = st.columns([1, 2])
    with cs1:
        use_setsu = st.checkbox("節間成績で補正する", value=True, key="t3_setsu",
                                help="kyotei.sakura.ne.jp の今節成績を取得し、好調な艇の確率を上げます。")
    with cs2:
        setsu_k = st.slider("節間成績の効き k（0で無効）", 0.0, 2.0, 0.6, 0.1, key="t3_setsu_k",
                            help="今節の平均着順が良い艇ほど確率を増幅。学習した重みではなくヒューリスティックな後段補正です。")
    ...
    if st.button("🚀 全レース解析", type="primary", use_container_width=True, key="t3_run"):
        if not sel_jcds:
            ...
            n_done = 0
            n_races = 0
            setsu_cache = {}     # jcd -> {rno: {lane: 今節成績}}
            for jcd in jcds:
                setsu_map = {}
                if use_setsu and setsu_k > 0:
                    if jcd not in setsu_cache:
                        setsu_cache[jcd] = fetch_setsu_results(dstr3, jcd)
                        time.sleep(0.15)
                    setsu_map = setsu_cache[jcd]
                for rno in range(1, 13):
                    n_done += 1
                    status.write(f"解析中… {JCD_NAME[jcd]} {rno}R ({n_done}/{total})")
                    prog.progress(n_done / total)
                    try:
                        base_res = fetch_race_data(dstr3, jcd, rno)
                    except Exception:
                        base_res = None
                    time.sleep(0.15)   # 取得先への節度
                    if not base_res:
                        continue
                    racers, lane_to_rank, _odds_map, payoff = base_res
                    n_races += 1
                    try:
                        feat_df = make_race_features(racers)
                    ...
                    # 節間成績（今節成績）でモデル確率を後段補正
                    if use_setsu and setsu_k > 0 and setsu_map.get(rno):
                        combo_probs = apply_setsu_adjustment(combo_probs, setsu_map[rno], setsu_k)
            ...
            st.success(f"解析完了 — 取得 {n_races:,} レース / 買い目あり {len(bet_records):,} レース"
                       f"（見送り {n_skip:,}） / 勝率≥{win_th:.1f}% の買い目 {len(hits):,} 点")
            if use_setsu and setsu_k > 0:
                n_setsu_ok = sum(1 for v in setsu_cache.values() if v)
                st.caption(f"節間成績 補正: ON（k={setsu_k:.1f}・好調艇を増幅） / "
                           f"今節成績を取得できた場 {n_setsu_ok}/{len(jcds)}。"
                           "取得0の場はその場だけ無補正です（場名ローマ字や未掲載が原因のことがあります）。")
            else:
                st.caption("節間成績 補正: OFF（モデル素の確率で表示）。")
            ...
            if not hits:
                ...
                # --- 勝率しきい値以上の買い目 明細 ---
                st.subheader(f"🎯 勝率{win_th:.1f}%以上の買い目 明細")
                df_hits = (pd.DataFrame(hits)
                           .sort_values(["勝率(%)", "場", "R"], ascending=[False, True, True])
                           .reset_index(drop=True))
                st.dataframe(df_hits, use_container_width=True)
                st.caption("勝率の高い順。オッズ未取得のため EV は算出していません。")
