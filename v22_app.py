...
<style>
  :root { --gold:#d8b15c; --bg:#0b0f1e; --panel:#121830; }
  * { box-sizing:border-box; }
  body { margin:0; background:var(--bg); color:#e8e6df;
         font-family:"Hiragino Kaku Gothic ProN","Noto Sans JP",Meiryo,sans-serif; }
  .wrap { max-width:780px; margin:0 auto; padding:8px 10px 28px; }
  ...
  canvas { width:100%; display:block; border-radius:14px; margin-top:10px;
           background:#0a0e1c; }
  ...
  .btnStart { background:#e2403f; color:#fff; font-size:17px; font-weight:800;
              padding:12px 30px; border-radius:999px; min-width:140px; }
  .btnStart.run { background:#e2403f; }
  ...
  .vlab { width:118px; text-align:right; color:#aeb6cf; flex:none; }
...
<body><div class="wrap">
  ...
  <div class="btnRow">
    <button class="btnStart" id="bStart">▶ スタート</button>
    <button class="btnVerify" id="bVerify">📊 1000回検証（65%）</button>
    <button class="btnSpd" id="bSpd">速度 ×1</button>
  </div>
  <div class="phase" id="phase">▶ スタートでレース展開を1回シミュレーション。モデル確率に従って着順を抽選し、展開を再現します。</div>
  <div class="rankList" id="ranks"></div>
...
document.getElementById("hd").textContent =
  ...
  "　" + DATA.place + "　" + DATA.rno + "R　3周 1800m・左回り";
...
const CW = 760, CH = 400, R = 110, SL = 360;
const xL = (CW - SL) / 2, xR = xL + SL, cy = CH / 2;
const yT = cy - R, yB = cy + R;
const PER = 2 * SL + 2 * Math.PI * R;
function pos(u) { // 0..1 反時計回り。u=0 はホーム直線(下)右端=スタートライン
  let d = ((u % 1) + 1) % 1 * PER;
  if (d < SL) return { x: xR - d, y: yB, ang: Math.PI };               // 下直線 ←
  d -= SL;
  if (d < Math.PI * R) { const a = Math.PI / 2 + d / R;                 // 左ターン
    return { x: xL + R * Math.cos(a), y: cy + R * Math.sin(a), ang: a + Math.PI / 2 }; }
  d -= Math.PI * R;
  if (d < SL) return { x: xL + d, y: yT, ang: 0 };                      // 上直線 →
  d -= SL;
  const a = -Math.PI / 2 + d / R;                                       // 右ターン
  return { x: xR + R * Math.cos(a), y: cy + R * Math.sin(a), ang: a + Math.PI / 2 };
}
function drawCourse(label, lap) {
  ctx.clearRect(0, 0, CW, CH);
  // 水面
  ctx.lineWidth = 86; ctx.strokeStyle = "#0e2238"; ctx.lineCap = "round";
  ctx.beginPath();
  for (let i = 0; i <= 200; i++) { const p = pos(i / 200); i ? ctx.lineTo(p.x, p.y) : ctx.moveTo(p.x, p.y); }
  ctx.closePath(); ctx.stroke();
  ctx.lineWidth = 80; ctx.strokeStyle = "#103049"; ctx.stroke();
  // ターンマーク
  [[xL, cy], [xR, cy]].forEach(m => {
    ctx.beginPath(); ctx.arc(m[0], m[1], 7, 0, 7); ctx.fillStyle = "#e2403f"; ctx.fill();
    ctx.beginPath(); ctx.arc(m[0], m[1], 3, 0, 7); ctx.fillStyle = "#fff"; ctx.fill();
  });
  // スタート/ゴールライン
  ctx.strokeStyle = "#e2403f"; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.moveTo(xR - 4, yB - 42); ctx.lineTo(xR - 4, yB + 42); ctx.stroke();
  ctx.fillStyle = "#e2403f"; ctx.font = "bold 11px sans-serif"; ctx.textAlign = "center";
  ctx.fillText("START / GOAL", xR - 4, yB + 58);
  // 中央テキスト
  ctx.fillStyle = "#cdb87e"; ctx.font = "bold 14px sans-serif";
  ctx.fillText(DATA.place, CW / 2, cy - 26);
  ctx.fillStyle = "#6d7590"; ctx.font = "11px sans-serif";
  ctx.fillText("3周 1800m（左回り）", CW / 2, cy - 8);
  if (label) {
    ctx.fillStyle = "#141a36"; const w = ctx.measureText(label).width + 56;
    roundRect(CW / 2 - w / 2, cy + 6, w, 30, 8); ctx.fill();
    ...
    ctx.fillText(label, CW / 2, cy + 26);
  }
  if (lap) { ctx.fillStyle = "#8d93a8"; ctx.font = "bold 12px sans-serif"; ctx.fillText(lap, CW / 2, cy + 54); }
}
...
function drawBoat(b, t) {
  ...
  const off = (b.lane - 3.5) * 9 + Math.sin(t * 2.1 + b.wob) * 2.0;
  const x = p.x + nx * off, y = p.y + ny * off;
  ctx.strokeStyle = "rgba(255,255,255,0.18)"; ctx.lineWidth = 3;   // 引き波
  ctx.beginPath(); ctx.moveTo(x, y);
  ctx.lineTo(x + Math.cos(p.ang + Math.PI) * 20, y + Math.sin(p.ang + Math.PI) * 20); ctx.stroke();
  const c = COL[b.lane];
  ctx.beginPath(); ctx.arc(x, y, 13, 0, 7); ctx.fillStyle = c[0]; ctx.fill();
  ctx.strokeStyle = "rgba(255,255,255,.4)"; ctx.lineWidth = 1.5; ctx.stroke();
  ctx.fillStyle = c[1]; ctx.font = "bold 13px sans-serif"; ctx.textAlign = "center";
  ctx.fillText(b.lane, x, y + 4.5);
}
...
function newRace() {
  ...
  BOATS.forEach(b => {
    b.rank = rank[b.lane];
    b.T = T0 + (b.rank - 1) * (0.85 + Math.random() * 0.5) + Math.random() * 0.3;
    b.amp = (3.5 - b.rank) * 0.02 + (Math.random() - 0.5) * 0.05;   // 序盤の出入り
    b.st = 0.010 * (b.course - 1) + Math.random() * 0.008;          // 進入コースなりの出遅れ
    b.d = 0; b.wob = Math.random() * 6.28; b.done = false;
  });
  return { t: 0, order, goal: false };
}
function step(b, t) {
  const s = Math.min(t / b.T, 1.03);
  let f = s + b.amp * Math.sin(Math.PI * Math.min(s, 1)) - b.st * Math.max(0, 1 - s * 5);
  b.d = Math.max(0, Math.min(f, 1.03)) * 3;
  if (s >= 1) b.done = true;
}
function phaseText(maxd, goal) {
  if (goal) return "🏁 ゴール！";
  if (maxd < 0.05) return "スタート！";
  if (maxd < 0.30) return "1周1マーク — ここで展開がほぼ決まる！";
  const lap = Math.min(3, Math.floor(maxd) + 1);
  return lap >= 3 ? "最終周" : lap + "周目";
}
function renderRanks(sorted, fixed) {
  elRanks.innerHTML = sorted.map((b, i) => {
    const c = COL[b.lane];
    return '<div class="rrow' + (i === 0 ? ' p1' : '') + '">' +
      ...
      '<div class="sub">展示 ' + (b.tenji ? b.tenji.toFixed(2) : "—") +
      '　モデル1着率 ' + (W1[b.lane] * 100).toFixed(1) + '%</div></div>';
  }).join("");
}
function frame(ts) {
  if (!running) return;
  ...
  lastTs = ts;
  race.t += dt;
  BOATS.forEach(b => step(b, race.t));
  const sorted = BOATS.slice().sort((a, b) => (b.d - a.d) || (a.rank - b.rank));
  const maxd = sorted[0].d;
  if (BOATS.filter(b => b.rank <= 3).every(b => b.done) && maxd >= 3) {
    race.goal = true;
  }
  drawCourse(phaseText(maxd, race.goal), race.goal ? "" : "LAP " + Math.min(3, Math.floor(maxd) + 1) + " / 3");
  BOATS.slice().sort((a, b) => a.d - b.d).forEach(b => drawBoat(b, race.t));
  renderRanks(race.goal ? BOATS.slice().sort((a, b) => a.rank - b.rank) : sorted);
  elPhase.textContent = "";
  if (race.goal && BOATS.every(b => b.done)) {
    running = false;
...
bVerify.onclick = () => {
  ...
  document.getElementById("vTitle").textContent =
    "📊 1000回検証結果（予想反映度 " + slider.value + "%）— 各艇の1着回数";
...
drawCourse("", "");
BOATS.forEach(b => { b.d = 0; b.wob = Math.random() * 6.28; });
BOATS.slice().sort((a, b) => a.d - b.d).forEach(b => drawBoat(b, 0));
renderRanks(BOATS.slice().sort((a, b) => EVALS[b.lane] - EVALS[a.lane]));
</script></body></html>'''
