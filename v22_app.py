...
function buildScript(b) {     // ピットアウト→2Mを左回りに旋回→待機隊形
  const lane = b.lane, c = b.course;
  const slotX = xL - 150 + (lane - 1) * 21, slotY = yB + 42;
  const t0 = 0.3 + lane * 0.16;
  b.wx = c <= 3 ? LINEX - 104 - c * 9 : LINEX - 178 - (c - 3) * 17;   // スロー / ダッシュ(深め)
  b.wy = yB + (c - 3.5) * 11;
  b.keys = [
    [0, slotX, slotY], [t0, slotX, slotY],
    [t0 + 0.9, xL - 78, yB + 14],
    [t0 + 1.7, xL - 112, cy + 28],
    [t0 + 2.4, xL - 92, cy - 36],
    [t0 + 3.1, xL - 18, cy - 56],
    [T_PIT_END + c * 0.16, (b.wx + xL + 40) / 2, b.wy - 8],
    [T_FORM + c * 0.16, b.wx, b.wy],
  ];
}
function scriptPos(b, t) {
  ...
  for (let i = 0; i < k.length - 1; i++) {
    if (t <= k[i + 1][0]) {
      const s = smooth((t - k[i][0]) / (k[i + 1][0] - k[i][0]));
      return { x: k[i][1] + (k[i + 1][1] - k[i][1]) * s,
               y: k[i][2] + (k[i + 1][2] - k[i][2]) * s };
    }
  }
  return { x: k[k.length - 1][1], y: k[k.length - 1][2] };
}
...
function drawPond(t, label, lap) {
  ...
  ctx.beginPath(); ctx.arc(xL - 58, cy + 58, 4, 0, 7); ctx.fillStyle = "#9aa0b5"; ctx.fill();
  // ピット（2マーク後方）
  ctx.fillStyle = "#27314f"; roundRect(xL - 162, yB + 30, 140, 26, 6); ctx.fill();
  ctx.fillStyle = "#8d93a8"; ctx.font = "bold 10px sans-serif"; ctx.fillText("PIT", xL - 92, yB + 70);
  // 場名・フェーズ
...
function phaseText(t, maxp, goal) {
  if (goal) return "🏁 ゴール！";
  if (t < T_PIT_END) return "ピットアウト — 2マークを左回りに旋回";
