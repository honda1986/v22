...
with tab3:
    ...
    if st.button("🚀 全レース解析", type="primary", use_container_width=True, key="t3_run"):
        if not sel_jcds:
            ...
            st.success(f"解析完了 — 取得できたレース {n_races:,} 本 / "
                       f"勝率≥{win_th:.1f}% の買い目 {len(hits):,} 点")
            if hits:
                df_hits = (pd.DataFrame(hits)
                           .sort_values(["勝率(%)", "場", "R"],
                                        ascending=[False, True, True])
                           .reset_index(drop=True))
                n_target_races = df_hits[["場", "R"]].drop_duplicates().shape[0]
                st.subheader(f"🎯 勝率{win_th:.1f}%以上の買い目（該当 {n_target_races} レース）")
                st.dataframe(df_hits, use_container_width=True)
                st.caption("勝率の高い順に表示しています。"
                           "オッズ未取得のため EV は算出していません。")
            else:
