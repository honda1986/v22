...
with tab3:
    ...
    st.caption("⚠️ コース別補正ONは選手ごとにページ取得するため非常に重いです（選手数ぶんのリクエスト）。"
               "全場だと数百〜千リクエストになるので、場を絞ることを強く推奨します。")
    ...
    if st.button("🚀 全レース解析", type="primary", use_container_width=True, key="t3_run"):
        if not sel_jcds:
            ...
            rate_cache = {}      # 登録番号 -> {course: {'r2','r3'}}（選手ページのキャッシュ）
            for jcd in jcds:
                regno_map = {}
                if use_course and course_k > 0:
                    if jcd not in regno_cache:
                        regno_cache[jcd] = fetch_racelist_regnos(dstr3, jcd)
                        time.sleep(0.15)
                    regno_map = regno_cache[jcd]
                for rno in range(1, 13):
                    ...
                    if use_course and course_k > 0 and regno_map.get(rno):
                        ...
                        for lane in range(1, 7):
                            ...
                            rates = {}
                            if reg:
                                if reg not in rate_cache:
                                    status.write(f"選手データ取得… {JCD_NAME[jcd]} {rno}R {lane}号艇")
                                    rate_cache[reg] = fetch_course_rates(dstr3, reg)
                                    time.sleep(0.15)
                                rates = rate_cache[reg]
            ...
            if use_course and course_k > 0:
                ...
                st.caption(f"コース別補正: ON（k={course_k:.1f}・進入コースの連対率で増幅） / "
                           f"登録番号を取得できた場 {n_regno_ok}/{len(jcds)} / 取得した選手ページ {len(rate_cache):,} 件。"
