...
function buildScript(b) {     // ピットアウト→2Mを中心とした円弧を「逆時計回り」に旋回→待機隊形
  const lane = b.lane, c = b.course;
  b.slot = { x: xL - 146 + (lane - 1) * 21, y: 86 };          // ピットは2マーク後方(バック側)
  b.t0 = 0.3 + lane * 0.16;
  b.r2m = 34 + lane * 5;                                       // 旋回半径(艇ごとにずらして重なり防止)
  b.tArc0 = b.t0 + 0.8;                                        // 旋回開始
  b.tArc1 = b.tArc0 + 2.2;                                     // 旋回終了
  b.tForm = T_FORM + c * 0.16;                                 // 待機隊形完成
  b.wx = c <= 3 ? LINEX - 104 - c * 9 : LINEX - 178 - (c - 3) * 17;   // スロー / ダッシュ(深め)
  b.wy = yB + (c - 3.5) * 11;
}
const PH0 = -100 * Math.PI / 180, PH1 = -270 * Math.PI / 180;  // 2M上側→西→南側へ(画面の逆時計回り)
...
function drawPond(t, label, lap) {
  ...
  ctx.beginPath(); ctx.arc(xL - 22, cy - 20, 4, 0, 7); ctx.fillStyle = "#9aa0b5"; ctx.fill();
  ctx.fillStyle = "#6d7590"; ctx.font = "9px sans-serif";
  ctx.fillText("小回り防止ブイ", xL - 52, cy - 32);
  // ピット（2マーク後方・バック側）
  ctx.fillStyle = "#27314f"; roundRect(xL - 158, 66, 142, 24, 6); ctx.fill();
  ctx.fillStyle = "#8d93a8"; ctx.font = "bold 10px sans-serif"; ctx.fillText("PIT", xL - 170, 84);
  // 場名・フェーズ
