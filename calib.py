...
def main():
    ...
    ap.add_argument("--course", choices=["raw", "lane"], default="raw",
                    help="lane にすると進入を枠なりに固定する(本番進入の漏れを消す)")
    ...
    if args.course == "lane":
        say("★進入を枠なりに固定しています(本番進入の漏れを消した下限の測定)")
    if not races:
        sys.exit("該当レースなし")
    ...
    for i, r in enumerate(races):
        Pm[i] = A.combo_probs_fast(P.make_race_features(rows_of(r)), r["jcd"], cidx)
        Od[i] = r["odds"]
        Hit[i] = cidx[r["hit"]]
        Pay[i] = r["pay_3t"]
        Day[i] = int(r["date"])
