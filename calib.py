...
def main():
    ...
    say(f"前半 {int(tr.sum()):,}レース(〜{int(mid)})で推定 → 後半 {int(te.sum()):,}レースで評価")
    ...
    say(f"  前半の対数損失 {best:.4f}  →  後半 {nll(lp[te], lq[te], Hit[te], ga, gb):.4f}")
    if gb > ga * 1.5:
    ...
    tbl(["確率", "点/R", "後半点数", "前半", "後半", "誤差±", "z値"], rows)

    # 参考: 市場の1番人気を1点買い
    fav = np.argmax(Q, axis=1)
    for label, mask in (("前半", tr), ("後半", te)):
        w = (fav == Hit)[mask]
    ...
    say("\n" + "=" * 54)
    say("見るべきは『配合 × 後半』の行だけ。a,bは前半で決めたので、後半は素の期間外。")
