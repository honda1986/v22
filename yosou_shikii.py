...
def main():
    ...
    ODS = np.stack([OD[keys[i]][2][reo] for i in idx])  # (レース, 120)
    P = CP[idx]
    DT = date[idx]
    ...
    print("=" * 64)
    print("[資金配分を変える]  勝負レース × 予想確率5%以上")
    print("  1レースの予算を決めて、配分だけを変える")

    def bunpai(budget, mask, th=0.05):
        """A 均等 / B 払戻均等 / C 確率比例 の3通り"""
        res = {k: [] for k in "ABC"}
        cnt = 0
        for i in np.where(mask)[0]:
            sel = np.where(P[i] >= th)[0]
            if len(sel) == 0:
                continue
            cnt += 1
            o = ODS[i][sel]
            pr = P[i][sel]
            won = HI[i] in sel
            j = int(np.where(sel == HI[i])[0][0]) if won else -1
            st = {"A": np.full(len(sel), budget / len(sel)),
                  "B": budget * (1 / o) / (1 / o).sum(),
                  "C": budget * pr / pr.sum()}
            for k, v in st.items():
                res[k].append(v[j] * o[j] if won else 0.0)
        return cnt, res

    LAB = {"A": "均等買い", "B": "払戻を均等に", "C": "確率に比例"}
    for budget in (1000.0, 10000.0):
        cnt, res = bunpai(budget, shobu)
        print(f"\n  ◇ 1レース {budget:,.0f}円  ({cnt:,}レース)")
        out = []
        for k in "ABC":
            v = np.array(res[k])
            cost = cnt * budget
            ...
            w = v[v > 0]
            out.append([LAB[k], pc(roi), pc(se), f"{v.sum()-cost:+,.0f}",
                        f"{w.mean():,.0f}" if len(w) else "—",
                        f"{w.min():,.0f}" if len(w) else "—",
                        f"{w.max():,.0f}" if len(w) else "—",
                        f"{w.std()/w.mean():.2f}" if len(w) else "—"])
        tbl(["配分", "回収率", "誤差±", "収支", "平均払戻", "最小", "最大",
             "ばらつき率"], out)

    print("\n  ◇ 払戻を均等にしたとき、1万円が何円になるか")
    cnt, _ = bunpai(10000.0, shobu)
    Xs, hits = [], []
    for i in np.where(shobu)[0]:
        sel = np.where(P[i] >= 0.05)[0]
        if len(sel) == 0:
            continue
        o = ODS[i][sel]
        Xs.append(10000.0 / (1 / o).sum())
        hits.append(HI[i] in sel)
    Xs = np.array(Xs); hits = np.array(hits)
    out = []
    for lo, hi in ((0, 12000), (12000, 15000), (15000, 20000), (20000, 10**9)):
        m2 = (Xs >= lo) & (Xs < hi)
        if m2.sum() < 200:
            continue
        roi = (Xs[m2] * hits[m2]).sum() / (m2.sum() * 10000)
        v = Xs[m2] * hits[m2]
        se = np.sqrt(((v - roi * 10000) ** 2).sum()) / (m2.sum() * 10000)
        out.append([f"{lo:,}〜{hi:,}円" if hi < 10**8 else "20,000円〜",
                    f"{int(m2.sum()):,}", f"{Xs[m2].mean():,.0f}円",
                    pc(hits[m2].mean()), pc(roi), pc(se)])
    ...
    for y in sorted(set(yr.tolist())):
        mm = shobu & (yr == y)
        c2, r2 = bunpai(10000.0, mm)
        if c2 < 200:
            continue
        v = np.array(r2["B"])
        roi = v.sum() / (c2 * 10000)
        se = np.sqrt(((v - roi * 10000) ** 2).sum()) / (c2 * 10000)
        out.append([f"{y}年度", f"{c2:,}", pc(roi), pc(se),
                    f"{v.sum()-c2*10000:+,.0f}"])
    tbl(["年度", "レース", "回収率", "誤差±", "収支"], out)
