...
def main():
    ...
    for rid, g in df.groupby("race", sort=False):
        ...
        rows.append({
            ...
            "top3": tuple(int(x) + 1 for x in order[:3]),
            "gap34": float(ps[2] - ps[3]),
            "sum3": float(ps[:3].sum()),
            "o": o, "hi": hi, "pay": pay,
        })
    R = pd.DataFrame(rows)
    ...
    # ---------------- 5) 上位3艇ボックス ----------------
    print("\n" + "=" * 58)
    print("[5] 予想3位と4位に差があるとき、上位3艇を3連単ボックス(6点)で買う")
    print("  順番ではなく『どの3艇が上位に来るか』を当てにいく買い方")

    C = {c: i for i, c in enumerate(combos)}
    def box_idx(t):
        a, b, c = t
        return [C[f"{x}-{y}-{z}"] for x in (a, b, c) for y in (a, b, c)
                for z in (a, b, c) if len({x, y, z}) == 3]

    R["_box"] = R["top3"].map(lambda t: box_idx(t))
    R["_hit"] = [1 if R.at[i, "hi"] in R.at[i, "_box"] else 0 for i in R.index]
    R["_ret"] = R["_hit"] * R["pay"]
    ...
    def roi(m):
        ...
        if k < 200:
            return None
        ret = R.loc[m, "_ret"].values
        cost = np.full(k, 600.0)
        r = ret.sum() / cost.sum()
        se = np.sqrt(((ret - r * cost) ** 2).sum()) / cost.sum()
        return {"n": k, "hit": R.loc[m, "_hit"].mean(), "roi": r, "se": se,
                "z": (r - 1) / se, "pl": ret.sum() - cost.sum()}

    for name, col in (("3位と4位の差", "gap34"), ("上位3艇の確率合計", "sum3")):
        print(f"\n  ◇ {name}")
        qs = np.quantile(R[col], [0.2, 0.4, 0.6, 0.8])
        gs = [(f"〜{qs[0]:.3f}", R[col] < qs[0]),
              (f"{qs[0]:.3f}〜{qs[1]:.3f}", (R[col] >= qs[0]) & (R[col] < qs[1])),
              (f"{qs[1]:.3f}〜{qs[2]:.3f}", (R[col] >= qs[1]) & (R[col] < qs[2])),
              (f"{qs[2]:.3f}〜{qs[3]:.3f}", (R[col] >= qs[2]) & (R[col] < qs[3])),
              (f"{qs[3]:.3f}〜", R[col] >= qs[3])]
        out = []
        for lab, m in gs:
            s2 = roi(m)
            if s2:
                out.append([lab, f"{s2['n']:,}", pc(s2["hit"]), pc(s2["roi"]),
                            pc(s2["se"]), f"{s2['z']:+.1f}", f"{s2['pl']:+,.0f}"])
        tbl(["区分", "レース", "的中率", "回収率", "誤差±", "z値", "収支"], out)

    print("\n  ◇ 上位1割だけ買った場合(差が大きい順)")
    out = []
    for frac in (0.05, 0.10, 0.20, 0.30):
        th = np.quantile(R["gap34"], 1 - frac)
        s2 = roi(R["gap34"] >= th)
        if s2:
            out.append([f"上位{frac:.0%}", f"{s2['n']:,}", pc(s2["hit"]),
                        pc(s2["roi"]), pc(s2["se"]), f"{s2['z']:+.1f}",
                        f"{s2['pl']:+,.0f}"])
    tbl(["範囲", "レース", "的中率", "回収率", "誤差±", "z値", "収支"], out)

    print("\n  ◇ 前半後半 (差が上位20%のレース)")
    th = np.quantile(R["gap34"], 0.8)
    sel = R["gap34"] >= th
    half = R.loc[sel, "date"].median()
    out = []
    for lab, m in (("前半", sel & (R["date"] <= half)),
                   ("後半", sel & (R["date"] > half))):
        s2 = roi(m)
        if s2:
            out.append([lab, f"{s2['n']:,}", pc(s2["hit"]), pc(s2["roi"]),
                        pc(s2["se"]), f"{s2['z']:+.1f}"])
    tbl(["期間", "レース", "的中率", "回収率", "誤差±", "z値"], out)
