...
def add_features(df):
    ...
    for c in ("n_win", "l_win", "m_2ren", "c_win", "avg_st", "tok", "st_setsu"):
        ...
        df[f"{c}_rk"] = g[c].rank(ascending=(c in ("avg_st", "st_setsu")),
                                  method="min")
    # F2以上は別扱い(中日スポーツの分析: F1の影響はほぼ無い、F2は大きい)
    df["is_f2"] = (df["f_count"].fillna(0) >= 2).astype(float)
    df["has_f"] = (df["f_count"].fillna(0) >= 1).astype(float)

    # 内側の艇との差。大きいほど自分が優れている(STは符号反転)
    df = df.sort_values(["race", "lane"]).reset_index(drop=True)
    gg = df.groupby("race")
    for c, big in (("n_win", True), ("tok", True), ("m_2ren", True),
                   ("avg_st", False), ("st_setsu", False)):
        prev = gg[c].shift(1)
        df[f"{c}_in"] = (df[c] - prev) if big else (prev - df[c])
    g = df.groupby("race")
