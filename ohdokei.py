...
# 1レースにつきオッズは1回だけ取る。
#   GitHub Actions の schedule は5分指定でも実際は大きく遅延・スキップされる。
#   そこで「直近の実行間隔」から、次の実行までに締切を過ぎてしまうレースを
#   先回りして判定する。実行が疎なほど早めに判定する(精度は落ちるが取りこぼさない)。
WIN_IDEAL = 14        # 判定窓の上限(締切に引きつけるほどオッズが実測に近い)
WIN_LO = 6            # ここより後だと買う時間がない
LATE_LO = 3           # 窓を逃したときの拾い直し
HORIZON_CAP = 35      # これより早い判定はしない(オッズが固まっていない)
...
def main():
    ...
    if os.path.exists(args.out):
        try:
            ...
            if old.get("date") == today:
                ...
                data.setdefault("venue_scan_at", 0)
    ...
    # 1) 今日の開催と締切時刻
    #    深夜〜早朝はその日の予定がまだ出ていないので、空振りを保存しない。
    #    見つからなかった場合は一定時間おきに調べ直す。
    last = data.get("venue_scan_at", 0)
    since = (time.time() - last) / 60
    need_scan = (not data["venues"]) and (since >= 25)
    if need_scan:
        print("今日の開催を調べます")
        found = {}
        ...
        data["venue_scan_at"] = time.time()
        if found:
            data["venues"] = found
            print(f"  {len(found)}場が開催中です")
        else:
            print("  まだ予定が出ていません。25分後にもう一度調べます。")
    elif not data["venues"]:
        print(f"開催調査は{since:.0f}分前に空振り。25分あけて再挑戦します。")

    # 2) 判定するレースを選ぶ (1レース1回)
    # 直近の実行間隔から、今回どこまで先回りするかを決める
    gaps = data.get("gaps", [])
    gap_est = sorted(gaps[-10:])[len(gaps[-10:]) // 2] if gaps else 10.0
    horizon = min(HORIZON_CAP, max(WIN_IDEAL, WIN_LO + gap_est * 1.6))
    print(f"直近の実行間隔 {gap_est:.0f}分 → "
          f"締切{WIN_LO}〜{horizon:.0f}分前を判定対象にします"
          + ("  (上限で頭打ち。取りこぼす分は諦めます)"
             if horizon >= HORIZON_CAP else ""))
    ...
    for mins, jcd, rno, key, t, is_late in queue:
        ...
        cheap3 = sorted(range(120), key=lambda i: odds[i])[:3]
        base = {"jcd": jcd, "venue": VENUE[jcd], "rno": rno, "close": t,
                "net": deadline(now, t).strftime("%H:%M"),
                "top3": [{"c": COMBOS[i], "o": round(odds[i], 1)} for i in cheap3],
        ...
        if rec is None:
            ...
            print(f"  {VENUE[jcd]}{rno}R {t}締切 {round(mins)}分前  "
                  f"見送り(6倍以下{n_cheap}点 最安{base['top3'][0]['o']}倍)",
                  flush=True)
            continue
    ...
    chk = [(k, r) for k, r in data["races"].items()
           if r.get("status") and not r.get("final")]
    done_chk = 0
    for key, r in chk:
        if done_chk >= 6:
            break
        ...
        fo = [round(odds[CIX[p["c"]]], 1) for p in r.get("points", [])]
        r["final"] = {"odds": fo, "n_cheap": n_cheap_final,
                      "min": round(min(odds), 1),
                      "still": n_cheap_final == NEED}
        done_chk += 1
        if r["status"] == "buy":
            drift = [f"{p['o']}→{f}" for p, f in zip(r["points"], fo)]
            print(f"  締切時 {r['venue']}{r['rno']}R  {' / '.join(drift)}  "
                  f"6倍以下{n_cheap_final}点 "
                  f"{'条件維持' if r['final']['still'] else '条件外れ'}")
        else:
            was = r.get("top3", [{}])[0].get("o")
            mark = "★締切時なら該当" if n_cheap_final == NEED else ""
            print(f"  締切時 {r['venue']}{r['rno']}R 見送り  "
                  f"最安 {was}→{r['final']['min']}倍  "
                  f"6倍以下 {r['n_cheap']}→{n_cheap_final}点 {mark}")

    # 3) 結果を入れる
    need = {}
    for key, r in data["races"].items():
        if r.get("result") or not r.get("status"):
            continue
        if (now - deadline(now, r["close"])).total_seconds() / 60 > 15:
            need.setdefault(r["jcd"], []).append((key, r["rno"]))
    for jcd, items in list(need.items())[:10]:
        try:
        ...
        for key, rno in items:
            if rno not in res:
                continue
            rr = res[rno]
            rr_rec = data["races"][key]
            picks = [p["c"] for p in rr_rec.get("points", [])]
            rr_rec["result"] = {
                "hit": rr["hit"], "pay": rr["pay"],
                "won": bool(picks) and rr["hit"] in picks,
                "henkan": rr["henkan"]}
            tag = ("的中" if rr["hit"] in picks else "不的中") if picks else "見送り"
            print(f"  結果 {VENUE[jcd]}{rno}R {rr['hit']} {rr['pay']}円 {tag}")

    prev_run = data.get("last_run", 0)
    gap = (started - prev_run) / 60 if prev_run else None
    data["last_run"] = started
    fin = [r for r in data["races"].values() if r.get("final")]
    if len(fin) >= 5:
        miss = sum(1 for r in fin
                   if r["status"] == "skip" and r["final"]["n_cheap"] == NEED)
        keep = sum(1 for r in fin
                   if r["status"] == "buy" and r["final"]["still"])
        buys = sum(1 for r in fin if r["status"] == "buy")
        dm = [r["final"]["min"] - r.get("top3", [{}])[0].get("o", 0)
              for r in fin if r.get("top3")]
        print(f"\n判定時→締切時のズレ (照合{len(fin)}件)")
        if dm:
            print(f"  最安オッズの変化 中央値 {sorted(dm)[len(dm)//2]:+.1f}倍")
        print(f"  判定時は見送りだが締切時なら該当 {miss}件  ← 判定が早すぎる分")
        if buys:
            print(f"  買い目が締切時も条件維持 {keep}/{buys}件")

    data["updated"] = now.strftime("%Y-%m-%d %H:%M")
    if gap is not None:
        data["gap_min"] = round(gap, 1)
        mark = "  ★遅延" if gap > 12 else ""
        print(f"前回実行からの間隔 {gap:.1f}分{mark}")
        hist = data.get("gaps", [])
        hist.append(round(gap, 1))
        data["gaps"] = hist[-40:]
        if len(hist) >= 5:
            print(f"直近の間隔: {' '.join(f'{g:g}' for g in hist[-8:])}分")
