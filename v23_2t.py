...
def fetch(sess, d, jcd, rno):
    try:
        r = sess.get(URL, params={"rno": rno, "jcd": f"{jcd:02d}", "hd": d}, timeout=25)
...
def main():
    ...
    print("\n速度テスト(10件)…")
    t0 = time.time()
    for d, jcd, rno, _, _ in sample[:10]:
        fetch(sess, d, jcd, rno)
    per = (time.time() - t0) / 10
    print(f"  1件あたり {per:.1f}秒")
    if per > 3:
        est = per * len(sample) / WORKERS / 60
        print(f"  ★遅いです。{len(sample):,}件で約{est:.0f}分かかります。")
        if est > MINUTES:
            print(f"  {MINUTES}分で打ち切るので、取れるのは約"
                  f"{int(MINUTES*60/per*WORKERS):,}件です。")
    else:
        print("  → 十分速いです")

    # --- 取得 ---
    print(f"\n{len(sample):,}レースを取得(同時{WORKERS}, 上限{MINUTES}分)")
    O2, IDX = [], []
    fail = {}
    t0 = time.time()
    stop = False
    with ThreadPoolExecutor(max_workers=WORKERS) as ex:
        for i in range(0, len(sample), 200):
            ...
            res = list(ex.map(lambda s: fetch(sess, s[0], s[1], s[2]), chunk))
