        ...
        for r in rawd["races"]:
            ...
            for e in r["entries"]:
                ...
                rows.append((
                    ...
                    float(x.get("c_win") or 0),
                    float(x.get("c_ren3") or 0),
                    float(x.get("c_st") or 0),
                    float(x["tokuten"]),
...
def main():
    ...
    # ---------------- 2.6) コース別勝率 × 節ST ----------------
    print("\n" + "=" * 58)
    print("[2.6] コース別1着率 と 節STの掛け合わせ")
    print("  『節間スタートが速い艇を買う。ただし速くてもそのコースで下手なら買わない』")
    hc = base & (D["c_win"] > 0)
    print(f"  コース別成績が取れている行 {int(hc.sum()):,} / {int(base.sum()):,}")
    if hc.sum() < 5000:
        print("  ★データ不足。tokuten.py を --force で取り直してください")
    else:
        cw = D["c_win"]
        qs = np.quantile(cw[hc], [0.2, 0.4, 0.6, 0.8])
        g_cw = [(f"〜{qs[0]:.0f}%", cw < qs[0]),
                (f"{qs[0]:.0f}〜{qs[1]:.0f}%", (cw >= qs[0]) & (cw < qs[1])),
                (f"{qs[1]:.0f}〜{qs[2]:.0f}%", (cw >= qs[1]) & (cw < qs[2])),
                (f"{qs[2]:.0f}〜{qs[3]:.0f}%", (cw >= qs[2]) & (cw < qs[3])),
                (f"{qs[3]:.0f}%〜", cw >= qs[3])]
        show("コース別1着率(直近6ヶ月)", g_cw, hc)

        both = hc & (D["st_s"] > 0)
        if both.sum() >= 5000:
            st_med = np.median(D["st_s"][both])
            cw_med = np.median(cw[both])
            fast = D["st_s"] < st_med
            good = cw >= cw_med
            print(f"\n  ◇ 掛け合わせ (節ST中央値 {st_med:.3f} / "
                  f"コース別1着率中央値 {cw_med:.1f}%)")
            g_x = [("速い×コース得意", fast & good),
                   ("速い×コース苦手", fast & ~good),
                   ("遅い×コース得意", ~fast & good),
                   ("遅い×コース苦手", ~fast & ~good)]
            rows = []
            for label, g in g_x:
                ...
                if not s2:
                    continue
                rows.append([label, f"{s2['n']:,}", pc(s2["exp"]), pc(s2["act"]),
                             f"{s2['d']*100:+.2f}", f"{s2['z']:+.1f}",
                             pc(s2["roi"])])
            tbl(["区分", "行数", "較正の期待", "実測", "差(pt)", "z値",
                 "想定回収率"], rows)
            print("  『速い×得意』だけが飛び抜けていれば、掛け合わせに意味がある")
